...
$id = $_GET['id'] ?? null;
if (!$id) {
    setFlashMessage('Cliente não encontrado', 'error');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $success = $clienteModel->update($id, $data);
        
        if ($success) {
            setFlashMessage('Cliente atualizado com sucesso!', 'success');
            redirect('/clientes/edit.php?id=' . $id);
        } else {
    ...
    // Atualizar dados do cliente com os valores do POST
    foreach ($data as $key => $value) {
        $cliente[$key] = $value;
    }
}

$pageTitle = "Editar Cliente #" . $cliente['codigo_cliente'];
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-800">
                    <i class="fas fa-user-edit mr-2"></i>Editar Cliente #<?php echo htmlspecialchars($cliente['codigo_cliente']); ?>
                </h1>
            ...
            <form method="POST" class="space-y-6" id="clienteForm">
                    ...
                    <div class="flex space-x-4">
                        <label class="flex items-center">
                            <input type="radio" name="tipo" value="PF" 
                                   <?php echo ($cliente['tipo'] === 'PF') ? 'checked' : ''; ?>
                        ...
                        <label class="flex items-center">
                            <input type="radio" name="tipo" value="PJ" 
                                   <?php echo ($cliente['tipo'] === 'PJ') ? 'checked' : ''; ?>
                                   onchange="toggleTipoPessoa()"
                                   class="mr-2">
                ...
                <div class="border-t pt-6">
                    ...
                    <textarea name="observacoes" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Observações adicionais sobre o cliente..."><?php echo htmlspecialchars($cliente['observacoes'] ?? ''); ?></textarea>
                </div>
...
// Inicializar estado do formulário ao carregar
